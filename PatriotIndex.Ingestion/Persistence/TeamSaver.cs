...
public class TeamSaver(ILogger<TeamSaver> logger, IDbContextFactory<PatriotIndexDbContext> dbFactory)
{
    ...
    public async Task SaveAsync(Team team, CancellationToken ct)
    {
        ...
        {
            // insert venue
            if (team.VenueId != Guid.Empty && team.Venue != null)
            {
                await UpsertVenueAsync(team.Venue, ctx);
            }
            
            // insert team
            await UpsertTeamAsync(team, ctx);
            
            if (team.Colors != null)
            {
                await UpsertTeamColorsAsync(team.Id, team.Colors, ctx);
            }

            // update coaches
            foreach(var coach in team.Coaches) await UpsertCoachAsync(coach, ctx);

            // update players
            foreach(var player in team.Players) await UpsertPlayerAsync(player, ctx);
    ...
    private async Task UpsertTeamAsync(Team team, PatriotIndexDbContext ctx)
    {
    ...
    private async Task UpsertTeamColorsAsync(Guid teamId, TeamColors? teamColors, PatriotIndexDbContext ctx)
    {
