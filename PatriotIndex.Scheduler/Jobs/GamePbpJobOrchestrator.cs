    ...
    ILogger<GamePbpJobOrchestrator> logger)
{
    ...
    {
        using var activity =
            PatriotIndexTelemetry.Source.StartActivity("GamePbpJobOrchestrator.RunAsync", ActivityKind.Internal);

        logger.LogInformation("Starting Game Pbp Job Orchestrator");

        var gameIds =
            await gamesRepository.GetGamesByTypeAndYearAsync("REG", 2025,
                CancellationToken.None); // returns all 32 team ids
