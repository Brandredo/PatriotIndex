...
public class SeasonalStatsJob(
    SportsApiClient apiClient,
    ILogger<SeasonalStatsJob> logger,
    ...
    StatsRepository statsRepository)
{
    ...
    {
        logger.LogInformation("Starting SeasonalStatsJob for team {TeamId} ({Year}/{SeasonType})", teamId,
            seasonInput.SeasonYear, seasonInput.SeasonType);

        using var activity =
            PatriotIndexTelemetry.Source.StartActivity("SeasonalStatsJob.RunAsync", ActivityKind.Internal);
        ...
        try
        {
            ...
            if (rawResponse == null)
            {
                ...
                data = await apiClient.GetAsync<SeasonalStatisticsDto>(
                    $"seasons/{seasonInput.SeasonYear}/{seasonInput.SeasonType}/teams/{teamId}/statistics.json",
                    cancellationToken);
